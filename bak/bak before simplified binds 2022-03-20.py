...
def moveChoice():
    ...
    button = read_hotkey(True) #takes a hotkey input and saves it as button
    if button == "shift+N": endPoint(userName, userScore, userContact)

    if keyMap[4] not in button:
        if button == keyMap[0]: 
    ...
    elif keyMap[4] in button:
        button = button[button.find(keyMap[4][-1])+2:]
...
def asignButtons():
    ...
    if "1" in chooseMap or "wasd" in chooseMap: 
        ...
        def mapToUp():
            ...
            keyMap.append(read_hotkey(False))
        def mapToLeft():
            global keyMap
        ...
        def mapToSpeed():
            global keyMap
            print("", end=f"\n\n\tPress the key you wish to be the :speed-up: ")
            keyMap.append(read_hotkey(False))
