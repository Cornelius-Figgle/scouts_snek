from qol_mth import sleep, clearConsole
...
def moveChoice():
    ...
    if spectrumMode == False: #if the user wants default controls (WASD)
        if button == "w": borderHit = True if playArea[snekPos[0] -  columns] == "═" else False ; moveUp() if borderHit == False else sleep(1) 
        elif button == "s": borderHit = True if playArea[snekPos[0] +  columns] == "═" else False ; moveDown() if borderHit == False else sleep(1)
        elif button == "a": moveLeft()
        elif button == "d": moveRight()
        elif button == "W": borderHit = True if playArea[snekPos[0] -  columns] == "═" else False ; moveUp() if borderHit == False else sleep(1) ; moveUp() if borderHit == False else sleep(1) 
        elif button == "S": borderHit = True if playArea[snekPos[0] +  columns] == "═" else False ; moveDown() if borderHit == False else sleep(1) ; moveDown() if borderHit == False else sleep(1) 
        ...
        if button == "w": borderHit = True if playArea[snekPos[0] -  columns] == "═" else False ; moveUp() if borderHit == False else sleep(1) 
        elif button == "a": borderHit = True if playArea[snekPos[0] +  columns] == "═" else False ; moveDown() if borderHit == False else sleep(1)
        elif button == "o": moveLeft()
        elif button == "p": moveRight()
        elif button == "W": borderHit = True if playArea[snekPos[0] -  columns] == "═" else False ; moveUp() if borderHit == False else sleep(1) ; moveUp() if borderHit == False else sleep(1) 
        elif button == "A": borderHit = True if playArea[snekPos[0] +  columns] == "═" else False ; moveDown() if borderHit == False else sleep(1) ; moveDown() if borderHit == False else sleep(1) 
...
def autoMoveChoice(button, direction):
    ...
    if spectrumMode == False: #if the user wants the defualt controls (WASD)
        if button == "w": borderHit = True if playArea[snekPos[0] -  columns] == "═" else False ; direction = 1 if borderHit == False else sleep(1) 
        elif button == "s": borderHit = True if playArea[snekPos[0] +  columns] == "═" else False ; direction = 2 if borderHit == False else sleep(1)
        elif button == "a": direction = 3
        elif button == "d": direction = 4
        elif button == "W": borderHit = True if playArea[snekPos[0] -  columns] == "═" else False ; direction = 5 if borderHit == False else sleep(1) ; direction = 5 if borderHit == False else sleep(1) 
        elif button == "S": borderHit = True if playArea[snekPos[0] +  columns] == "═" else False ; direction = 6 if borderHit == False else sleep(1) ; direction = 6 if borderHit == False else sleep(1) 
        elif button == "A": direction = 7 
        elif button == "D": direction = 8     
    else: #if the user wants the controls to be WAOP rather than WASD
        if button == "w": borderHit = True if playArea[snekPos[0] -  columns] == "═" else False ; direction = 1 if borderHit == False else sleep(1) 
        elif button == "a": borderHit = True if playArea[snekPos[0] +  columns] == "═" else False ; direction = 2 if borderHit == False else sleep(1)
        elif button == "o": direction = 3
        elif button == "p": direction = 4
        elif button == "W": borderHit = True if playArea[snekPos[0] -  columns] == "═" else False ; direction = 5 if borderHit == False else sleep(1) ; direction = 5 if borderHit == False else sleep(1) 
        elif button == "A": borderHit = True if playArea[snekPos[0] +  columns] == "═" else False ; direction = 6 if borderHit == False else sleep(1) ; direction = 6 if borderHit == False else sleep(1) 
...
def moveUp(): 
    ...
    for i in range(1, len(snekPos) + 1, 2): #alternate of above code
        try: snekPos.pop(i) ; snekPos.insert(i, snekPos[i - 1]) #remove the 'where next' items and replaces it with the current loc?????
        except: break
    for i in range(0, len(snekPos) + 1, 2): #alternate of previous, same as original
        try: snekPos.pop(i) ; snekPos.insert(i, snekPos[i] - columns) #conditional for direction, removes the item w. the current loc of snke and replaces it w. the loc of one space above in the terminal
        except: break
...
def moveDown():
    ...
    for i in range(1, len(snekPos) + 1, 2):
        try: snekPos.pop(i) ; snekPos.insert(i, snekPos[i - 1])
        except: break
    for i in range(0, len(snekPos) + 1, 2):
        try: snekPos.pop(i) ; snekPos.insert(i, snekPos[i] + columns)#conditional
        except: break
...
def checkFruit(fruitLoc):
    if playArea[fruitLoc] == "♦": #if the next loc is where the fruit is
...
def endPoint(name, score): file.close() ; clearConsole() ; print(f"\n\tThe End! You got a score of {userScore}\n\n\t\t%%Reset in 7_"); log = open("Scores.txt", "a") ; log.write(f"\n{name} {score}") ; log.close()
